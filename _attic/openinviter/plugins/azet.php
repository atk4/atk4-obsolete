...
class azet extends OpenInviter_Base
	...
	public function login($user,$pass)
		...
		$res=$this->get("http://www.email.azet.sk/");
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"http://www.email.azet.sk/",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"http://www.email.azet.sk/",'GET',false);	
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
	...
	public function getMyContacts()
		...
		if (isset($contacts_email)) foreach($contacts_email as $key=>$value) if (isset($contacts_name[$key])) $contacts[trim(preg_replace('/[^(\x20-\x7F)]*/','',(string)$value))]=$contacts_name[$key];
